...
class PoissonCompositeObjective:
    ...
    def u_boundary_current(self, smooth=False):
        if self.xb is None:
            return None
        #self._set_parameters(theta)
        #xb = self.xb
        if "smooth" in self.model.forward.__code__.co_varnames:
            return self.model(self.xb,smooth=bool(smooth))
        else:
            return self.model(self.xb)
    
    def _ub_of_params_functorch_smooth(self,params):
        if self.xb is None:
            return None
        ub = self._u_smooth(params, self.xb, True)
        return ub.reshape(-1)
    ...
    def value(self, theta, ftol=1e-12):
        with torch.enable_grad():
            ...
            if (self.xb is not None) and (self.lam_bc > 0.0):
                ub = self.u_boundary_current(smooth=False)
    ...
    def _smooth_act(self,x):
        ...
        """
        beta = getattr(self, "smooth_beta", 20.0)
    ...
    def apply_Jub_functorch_smooth(self, theta, s):
        params0 = {k: theta.td[k] for k,_ in self.model.named_parameters()}
        tang = {k: s.td[k]     for k, _ in self.model.named_parameters()}
        ub0, Jubv = jvp(self._ub_of_params_functorch_smooth, (params0,), (tang,))
        return Jubv
    
    def apply_JubT_functorch_smooth(self, theta,cotangent_ub):
        params0 = {k: theta.td[k] for k, _ in self.model.named_parameters()}
        ub0, pullback = vjp(self._ub_of_params_functorch_smooth, params0)
    ...
    def hessVec(self, v, theta, gradTol=1e-12):
        ...
        with torch.enable_grad():
            ...
            #SOFT BOUNDARY PART Hessian
            if (self.xb is not None) and (self.lam_bc > 0.0):
                Jubv = self.apply_Jub_functorch_smooth(theta, v) 
                if self.wb is None:
                    Nb = Jubv.numel()
                    ...
                    hv_bc = self.apply_JubT_functorch_smooth(theta,cot)
                    hv = hv+hv_bc
    ...
    def gradient_gn_smooth(self, theta):
        ...
        if (self.xb is not None) and (self.lam_bc > 0.0):
            params0 = {k: theta.td[k] for k,_ in self.model.named_parameters()}
            ub = self._ub_of_params_functorch_smooth(params0) 
            ...
            if self.wb is None:
                ...
                w = self.wb.reshape(-1) if self.wb.ndim == 2 else self.wb
                cot = self.lam_bc * (w * r)
            g_bc = self.apply_JubT_functorch_smooth(theta, cot)
            g = g+g_bc
...
def trustregion(x0, Deltai, problem, params):
    ...
    for i in range(1, params['maxit'] + 1):
        ...
        #Add for relu
        Fcur = val + phi
        Ftrial = valnew + phinew_true
        Fref = max(Fhist) if len(Fhist) > 0 else Fcur
        aRed_cur      = Fcur - Ftrial
        aRed_ref = Fref - Ftrial
        pRed      = float(pRed)
        pRed_floor = max(1e-12, 1e-10 * max(1.0, abs(Fref)))
        pRed_eff = max(pRed, pRed_floor)
        rho_cur = -np.inf if pRed <= 0 else aRed_cur / pRed_eff
        rho_ref = -np.inf if pRed <= 0 else aRed_ref / pRed_eff
        
        eta1_nm = params.get("eta1_nm", params["eta1"]) 
        improves_current = (Ftrial <= Fcur)
        allowed_nm       = (Ftrial <= Fref)
        #rho = -np.inf if pRed <= 0 else aRed / pRed_eff
        accept = (improves_current and rho_cur >= params["eta1"]) or \
         ((not improves_current) and allowed_nm and rho_ref >= eta1_nm)
        
        
        print("debug:","Fcur=", float(Fcur), "Fref=", float(Fref), "Ftrial=", float(Ftrial),"aRed=", float(aRed), "pRed=", float(pRed), "rho_cur=", float(rho_cur),"rho_ref=", float(rho_ref), "accept=", bool(accept))
        if not accept:
