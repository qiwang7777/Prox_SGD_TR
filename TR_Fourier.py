...
class PoissonCompositeObjective:
    ...
    def _ub_of_params_functorch(self,params):
        if self.xb is None:
            return None
        ub = self._u_smooth(params, self.xb, False)
        return ub.reshape(-1)
    ...
    def _f_of_params_functorch(self, params):
        ...
        buffers = dict(self.model.named_buffers())
        u = functional_call(self.model, (params, buffers), (xy,))
    ...
    def apply_Jub_functorch(self, theta, s):
        params0 = {k: theta.td[k] for k,_ in self.model.named_parameters()}
        tang = {k: s.td[k]     for k, _ in self.model.named_parameters()}
        ub0, Jubv = jvp(self._ub_of_params_functorch, (params0,), (tang,))
        return Jubv
    
    def apply_JubT_functorch(self, theta,cotangent_ub):
        params0 = {k: theta.td[k] for k, _ in self.model.named_parameters()}
        ub0, pullback = vjp(self._ub_of_params_functorch, params0)
    ...
    #---------------- Smoothing Hessian ----------------
    def hessVec_d(self, v, theta, gradTol=1e-12):
        with torch.enable_grad():
        # --- interior GN curvature ---
            Jv = self.apply_Jf_functorch(theta, v)          # use ReLU Jf
            N = self.xy.shape[0]
            d = self.xy.shape[1]
            kap = self.kappa_fn(self.xy)
            if kap.ndim == 1:
            ...
            hv = self.apply_JfT_functorch(theta, Hz)        # use ReLU Jf^T

        # --- boundary GN curvature ---
            if (self.xb is not None) and (self.lam_bc > 0.0):
                Jubv = self.apply_Jub_functorch(theta, v)   # use ReLU Jub
                if self.wb is None:
                    Nb = Jubv.numel()
...
def trustregion(x0, Deltai, problem, params):
    ...
    for i in range(1, params['maxit'] + 1):
        ...
        rho = -np.inf if pRed <= 0 else aRed / pRed

        
        allowed_nm       = (Ftrial <= Fref)
        
        accept =(rho >= params['eta1']) #and allowed_nm
