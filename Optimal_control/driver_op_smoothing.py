...
def solve_optimal_control_with_TR_smoothing(
    ngrid=64,
    alpha=1e-2,
    u_a=-1e6,
    u_b=1e6,
    ...
    # Smoothing controls
    params["use_smoothing_at_deltamin"] = use_smoothing_at_deltamin
    params["mu_smooth"] = mu_smooth
    params["mu_min"] = mu_min
    params["mu_factor"] = mu_factor
    params["mu_power"] = mu_power
    params["deltamin"] = delta_min

    if delta_smooth_exit is None:
        delta_smooth_exit = 5.0 * delta_min

    params["delta_smooth_exit"] = delta_smooth_exit
    params["smooth_mode"] = False

    x_opt, cnt, best_x = trustregion(
        x0,
        delta0,
        problem,
        params,
